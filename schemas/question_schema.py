...
class UserQuestion(BaseModel):
    ...
    id: str = Field(description="질문 고유 ID (q1, q2, q3 형태)")
    category: Literal[
        "online",  # 비대면가입
        "bank_app",  # 은행앱사용
        ...
        "using_card",  # 카드사용
        "first_banking",  # 첫거래
    ] = Field(description="우대조건 카테고리 (영문 코드)")
    question: str = Field(
...
class QuestionGeneratorResult(BaseModel):
    ...
    questions: list[UserQuestion] = Field(description="생성된 사용자 질문 목록")
...
class UserResponse(UserQuestion):
    ...
    response_value: bool = Field(description="조건 충족 여부 (True/False)")

    raw_response: str | None = Field(
        default=None, description="사용자 원본 응답 텍스트"
    )
...
class UserInputResult(BaseModel):
    ...
    user_responses: list[UserResponse] = Field(description="질문별 사용자 응답 목록")

    response_summary: dict[str, bool] = Field(description=" 질문별 응답 요약")
